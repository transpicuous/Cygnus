...
public class ClientSessionManager extends ChannelInboundHandlerAdapter {
    ...
    public void channelActive(ChannelHandlerContext ctx) {
        ...
        CClientSocket client = new CClientSocket(ch, SendSeq, RecvSeq);
        ...
        oPacket.Encode(0);
        client.SendPacket(oPacket.ToPacket());
        
        ch.attr(CClientSocket.SESSION_KEY).set(client);

        client.PingTask = ctx.channel().eventLoop().scheduleAtFixedRate(()
                -> client.SendPacket(CLogin.Ping()), 5, 5, TimeUnit.SECONDS);
    ...
    public void channelInactive(ChannelHandlerContext ctx) {
        ...
        CClientSocket client = (CClientSocket) ch.attr(CClientSocket.SESSION_KEY).get();
        client.Close();
        if (client.PingTask != null) {
            client.PingTask.cancel(true);
        }
